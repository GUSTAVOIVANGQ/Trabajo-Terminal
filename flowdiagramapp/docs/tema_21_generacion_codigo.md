...
El generador de código se implementa en [lib/compiler/code_generator_advanced.dart](../lib/compiler/code_generator_advanced.dart). Esta es la fase final del conversor que produce código C a partir del árbol de sintaxis abstracta optimizado.

---

## 21.1 Fundamentos de la Generación de Código

La generación de código constituye la quinta y última fase del conversor FlowCode, donde el árbol de sintaxis abstracta optimizado se transforma en código fuente C textual. A diferencia de compiladores tradicionales que producen código máquina o bytecode, FlowCode implementa un conversor que traduce representaciones visuales (diagramas de flujo) a código fuente legible.

Esta decisión arquitectónica responde al propósito de la herramienta. Los usuarios pueden examinar el código C generado línea por línea, compilarlo con GCC o Clang en su entorno de desarrollo, ejecutarlo para verificar su comportamiento, y depurarlo usando herramientas estándar como GDB. Este ciclo completo de diseñar-generar-compilar-ejecutar refuerza la conexión entre el diseño algorítmico visual y la programación textual.

El código generado sigue las convenciones del estándar C99, elegido por ser un estándar maduro, ampliamente soportado, y suficientemente expresivo para los conceptos de programación fundamental que FlowCode maneja. El generador opera sobre dos estructuras de datos principales: el AST optimizado que describe la lógica del programa, y la tabla de símbolos que contiene información de tipos y declaraciones de las variables.

---

## 21.2 Opciones de Generación

El generador ofrece opciones configurables que permiten adaptar el código producido a diferentes contextos de uso.
...
La opción de **marca de tiempo** añade un comentario en el encabezado indicando cuándo se generó el código. Esto es útil para mantener registro de versiones cuando el usuario itera sobre el diseño de su diagrama.

La **indentación** predeterminada usa cuatro espacios siguiendo las convenciones más comunes en C, aunque puede ajustarse según las preferencias del usuario.

El **modo debug** incrementa la verbosidad de los comentarios e incluye información adicional sobre el proceso de generación, útil principalmente durante el desarrollo y pruebas del propio conversor.
...
Este enfoque de recorrido por grafo garantiza que el código generado preserve fielmente la semántica del diagrama independientemente de cómo el usuario haya posicionado visualmente los nodos en el editor.
...
Esta correspondencia directa permite que los usuarios desarrollen un modelo mental claro de cómo cada elemento visual se traduce a código textual.
...
Para **declaraciones múltiples**, el generador soporta que el usuario escriba `int a, b, c` en un solo nodo. Esto se traduce directamente a una declaración múltiple en C con el formato apropiado.
...
Una característica importante del generador es la **normalización de operadores**. Dado que muchos usuarios de FlowCode están más familiarizados con español, el generador transforma automáticamente operadores escritos en lenguaje natural a su sintaxis C equivalente:
...
Esta normalización permite que los usuarios expresen condiciones de forma más natural, mientras el generador produce código C sintácticamente correcto.
...
Esta implementación asume que las funciones referenciadas están definidas externamente, ya sea en bibliotecas estándar de C o en archivos que el usuario proporcionará. Esto permite practicar el concepto de modularización y reutilización de código sin la complejidad adicional de definir funciones dentro del diagrama.

---

## 21.12 Métricas y Resultado de la Generación

El generador recopila estadísticas durante el proceso de traducción que proporcionan retroalimentación útil para diagnóstico técnico.
...
El resultado completo de la generación empaqueta el código C producido junto con todas las métricas y cualquier mensaje de error o advertencia. Este resultado estructurado permite que la interfaz de usuario de FlowCode presente al usuario no solo el código generado, sino también información contextual sobre su calidad y características.

El código generado está listo para compilarse directamente con cualquier compilador C compatible con C99 sin necesidad de modificaciones manuales, permitiendo que el usuario inmediatamente pruebe y ejecute su algoritmo.
