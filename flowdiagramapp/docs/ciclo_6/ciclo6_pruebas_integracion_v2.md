...
Las pruebas marcadas con ❌ documentan funcionalidades fuera del alcance definido en el protocolo del trabajo terminal o condicionadas a recursos no disponibles en el contexto de un proyecto de desarrollador individual en dos semestres académicos. Su inclusión tiene como propósito delimitar explícitamente las fronteras del sistema y dejar constancia de qué aspectos quedan como trabajo a futuro.

| Componente                                       | Tipo                       | Casos documentados en este reporte | Casos de uso cubiertos | Estado  |
| ------------------------------------------------ | -------------------------- | ---------------------------------: | ---------------------- | ------- |
| Análisis léxico                                | Unitaria                   |                                  8 | CU04                   | ✅      |
| Análisis sintáctico                            | Unitaria                   |                                  8 | CU04                   | ✅      |
| Análisis semántico                             | Unitaria                   |                                  7 | CU05                   | ✅      |
| Optimización y generación de código           | Unitaria                   |                                 10 | CU06                   | ✅      |
...
| Almacenamiento local y nube                      | Manual                     |                                 12 | CU03, CU08–CU10       | ✅ / ❌ |
...
| LEX-T07 | Reconocimiento de directivas de preprocesador (`#define`, `#include`)                   | Texto con directiva de preprocesador          | No aplica: las directivas de preprocesador están fuera del alcance del subconjunto C soportado; el sistema las reporta como unidad léxica no reconocida                                    | ✅     |
| LEX-T08 | Cadena de formato con especificadores combinados (`%d %f %s`) en un único nodo de salida | Nodo de salida con múltiples especificadores | No implementado: el analizador léxico procesa un especificador de formato por variable; la combinación de múltiples especificadores en una sola expresión está fuera del alcance actual | ✅     |

**Tabla 106.** Pruebas — Analizador léxico.

> **Nota:** LEX-T07 y LEX-T08 documentan limitaciones del alcance definido en el protocolo. El subconjunto C soportado (variables, E/S estándar, estructuras de control) no contempla directivas de preprocesador ni cadenas de formato compuestas. Estas funcionalidades se identifican como trabajo a futuro.
...
| SYN-T07 | Llamada a función con múltiples parámetros definida por el usuario | Llamada a función con dos argumentos    | No implementado: las funciones definidas por el usuario están fuera del alcance del protocolo; el analizador las reporta como construcción no soportada           | ✅     |
| SYN-T08 | Expresión ternaria (`condición ? valor_a : valor_b`)              | Expresión con operador ternario         | No implementado: el operador ternario no forma parte del subconjunto de estructuras de control soportadas; se representa mediante nodos de decisión en el diagrama | ✅     |

**Tabla 107.** Pruebas — Analizador sintáctico.

> **Nota:** SYN-T07 y SYN-T08 documentan construcciones sintácticas de C que exceden el alcance del protocolo. Las funciones definidas por el usuario y el operador ternario no tienen representación en el subconjunto de símbolos ISO 5807 soportado. Se identifican como trabajo a futuro.
...
| SEM-T07 | Verificación de tipo de retorno de función definida por el usuario | Función con tipo de retorno declarado                | No implementado: el análisis semántico de funciones definidas por el usuario está fuera del alcance del protocolo; el sistema no verifica compatibilidad de tipo de retorno | ✅     |

**Tabla 108.** Pruebas — Analizador semántico.

> **Nota:** SEM-T07 y SEM-T08 documentan verificaciones semánticas que exceden el alcance del protocolo. El análisis de funciones definidas por el usuario y la verificación estática de límites de arreglos no están contemplados en el subconjunto C soportado. Se identifican como trabajo a futuro.
...
| GEN-T06 | Generación de código para funciones definidas por el usuario     | Diagrama con subproceso que representa una función propia | No implementado: la generación de funciones definidas por el usuario está fuera del alcance del protocolo; los subprocesos se mapean a comentarios `/* llamada a función */` | ✅     |
| GEN-T07 | Generación de código con memoria dinámica (`malloc`/`free`) | Nodo de proceso con reserva de memoria                     | No implementado: la memoria dinámica está explícitamente excluida del alcance del protocolo; el estándar ISO 5807 no define un símbolo específico para esta operación      | ✅     |
...
> **Nota:** GEN-T06 y GEN-T07 documentan construcciones de generación de código fuera del alcance definido en el protocolo. La generación de funciones de usuario y memoria dinámica requeriría ampliar el conjunto de símbolos soportados y el analizador semántico más allá de lo factible en el plazo del trabajo terminal.
...
| CU07-T03 | MET-01.3                            | Exportación del diagrama como imagen png,jpg | Se genera correctamente la imagen exportada reflejando el lienzo visual del usuario | ✅     |
| CU07-T04 | —                                  | Exportación del codigo .c, .json             | Los archivos se exportan de forma local con la estructura esperada                   | ✅     |
...
| CU08-T04 | Creación de carpetas con más de cinco niveles de anidación | No evaluado: el protocolo establece un límite de cinco niveles; la prueba de exceder este límite no fue ejecutada porque el caso de uso ya define el comportamiento esperado (advertencia al usuario) | ❌     |
...
| CU10-T03 | Sincronización simultánea desde dos dispositivos con el mismo usuario | No evaluado: la prueba requiere dos dispositivos físicos; el manejo de conflictos de versión está definido en el caso de uso (el más reciente sobrescribe) pero no fue verificado con dos dispositivos simultáneos por limitación de recursos | ❌     |
...
| Tipo                                            | Aprobadas (✅) | No implementadas / no evaluadas (❌) |        Total |
| ----------------------------------------------- | -------------: | -----------------------------------: | -----------: |
| Unitarias automatizadas (casos representativos) |             33 |                                    0 |           33 |
| Integración extremo a extremo                  |             33 |                                    0 |           33 |
| Verificación del código generado              |              6 |                                    0 |            6 |
| Manuales                                        |             10 |                                    2 |           12 |
| **Total**                                 |   **82** |                          **2** | **84** |
...
| Funciones definidas por el usuario             | —                   | Fuera del alcance del protocolo — GEN-T06, SYN-T07 |
| Memoria dinámica (reserva/liberación)        | —                   | Fuera del alcance del protocolo — GEN-T07          |
| Estructuras definidas por el usuario           | —                   | Fuera del alcance del protocolo                     |
| Operaciones con archivos (lectura/escritura)   | —                   | Fuera del alcance del protocolo                     |
| Recursión                                     | —                   | Fuera del alcance del protocolo — CU06-T07         |
