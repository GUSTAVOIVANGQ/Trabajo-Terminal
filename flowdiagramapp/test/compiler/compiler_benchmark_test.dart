...
_DiagramData _generateLinearDiagram(int nodeCount) {
  ...
  for (int i = 1; i < nodeCount - 1; i++) {
    final processTypes = [
      'int x${varIndex++} = $i',
      'x${varIndex - 1} = x${varIndex - 1} + 1',
      'x${varIndex - 1} = x${varIndex - 1} * 2',
    ];

    nodes.add(DiagramNode(
      id: 'linear_process_$i',
      type: NodeType.process,
      position: Offset(200, 50.0 + i * 80),
      text: processTypes[i % processTypes.length],
    ));
  }
...
_DiagramData _generateMixedComplexityDiagram(int nodeCount) {
  ...
  for (int i = 0; i < min(5, nodeCount - 2); i++) {
    nodes.add(DiagramNode(
      id: 'mixed_init_$i',
      type: NodeType.process,
      position: Offset(300, yPos.toDouble()),
